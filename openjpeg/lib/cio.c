...
#pragma mark - buffer -> value

void opj_write_bytes_BE(
   OPJ_BYTE * p_buffer,
   OPJ_UINT32 p_value,
   OPJ_UINT32 p_nb_bytes
   )
{
   //write de los ultimos bytes, independientemente del tamaño del OPJ_UINT32
...
//jpeg200 is big endian. LE values need to be written reversed
//bytes in the name of the method is confusing
void opj_write_bytes_LE(
   OPJ_BYTE * p_buffer,  //dst
   OPJ_UINT32 p_value,   //src
   OPJ_UINT32 p_nb_bytes //siz
   )
{
    //reverse write
    const OPJ_BYTE * l_data_ptr = ((const OPJ_BYTE *) &p_value) + p_nb_bytes - 1;//last byte
    OPJ_UINT32 i;
...
void opj_read_bytes_BE(
   const OPJ_BYTE * p_buffer,
   OPJ_UINT32 * p_value,
   OPJ_UINT32 p_nb_bytes
   )
{
    OPJ_BYTE * l_data_ptr = ((OPJ_BYTE *) p_value);//first byte
                                 ...
                                 opj_event_mgr_t * p_event_mgr)
{
    ...
    if (p_stream->m_status & OPJ_STREAM_STATUS_ERROR) return (OPJ_SIZE_T) - 1;
    for (;;) {
        l_remaining_bytes = p_stream->m_buffer_size - p_stream->m_bytes_in_buffer;
         if (l_remaining_bytes >= p_size) {//we have more memory than required
        ...
        if (l_remaining_bytes) {//we copy data and then do an actual read on the stream
        ...
        if (! opj_stream_flush(p_stream, p_event_mgr)) return (OPJ_SIZE_T) - 1;
    }
}
