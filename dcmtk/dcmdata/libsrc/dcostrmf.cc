...
offile_off_t DcmFileConsumer::write(const void *buf, offile_off_t buflen)
{
  ...
  //JF if (status_.good() && file_.open() && buf && buflen)
  if (status_.good() && buf && buflen)
  {
     ...
     */
#define DcmFileConsumer_MAX_CHUNK_SIZE 65534 /* dckv size 0xFFFE   33554432 32 MByte */
...
void DcmFileConsumer::flush()
{
  // nothing to flush
   ...
   OFstatic_cast(offile_off_t, file_.fwrite(buf3, 1, OFstatic_cast(size_t, 12)));
