...
public:
  ...
  {
    //JF return STDIO_NAMESPACE fwrite(ptr, size, n, file_);
    return STDIO_NAMESPACE fwrite(ptr, size, n, stdout);
  }
  ...
  int fflush()
  {
    //JF int result = STDIO_NAMESPACE fflush(file_);
    int result = STDIO_NAMESPACE fflush(stdout);
  ...
  int eof() const
  {
    ...
    // feof is a macro on some systems. Macros never have namespaces.
     //JF return feof(file_);
     //const char buf3[]={ 0xFC , 0xFF , 0xFC , 0xFF , 0x4F , 0x42 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 };
     //written = OFstatic_cast(offile_off_t, file_.fwrite(buf3, 1, OFstatic_cast(size_t, 12)));
     return feof(stdout);
#else
     //JF return STDIO_NAMESPACE feof(file_);
     return STDIO_NAMESPACE feof(stdout);
#endif
  }
  ...
  int error()
  {
    ...
    return ferror(stdout);//JF file_
#else
    return STDIO_NAMESPACE ferror(stdout);//JF file_
#endif
  }
   ...
   */
#ifdef fileno
  int fileNo() { return fileno(stdout); }//JF file_
#else
  int fileNo() { return :: fileno(stdout); }//JF file_
#endif
  ...
  {
    int result = STDIO_NAMESPACE setvbuf(stdout, buf, modes, n);//file_
    if (result) storeLastError();
    return result;
  }
   ...
   */
  int fputc(int c) { return STDIO_NAMESPACE fputc(c, stdout); }//file_
   ...
   */
  int fputs(const char *s) { return STDIO_NAMESPACE fputs(s, stdout); }//file_
